...
class dk(keras.Model):
    def __init__(self, scope: str = "dk", k: int = 16, reg: float = 0.0005, norm: str = "instance"):
        ...
        self.norm = norm
        self.conv1 = conv(numout=k, kernel_size=3, strides=2, kernel_regularizer=reg, padding='same', name='conv')
        if norm == 'instance':
            self.scale = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='scale')
            self.offset = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='offset')
        elif norm == 'bn':
...
class uk(keras.Model):
    def __init__(self, scope: str = "uk", k: int = 16, reg: float = 0.0005, norm: str = "instance"):
        ...
        # self.conv1 = convt(numout=k, kernel_size=3, strides=2, kernel_regularizer=reg, padding='same', name='conv')
        if norm == 'instance':
            self.scale = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='scale')
            self.offset = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='offset')
        elif norm == 'bn':
    ...
    def call(self, x, training=False):
        height = x.shape[1]
        width = x.shape[2]
        x = tf.compat.v1.image.resize_images(x, [2 * height, 2 * width], method=1, align_corners=True)
...
class Ck(keras.Model):
    def __init__(self, scope: str = "ck", k: int = 16, stride: int = 2, reg: float = 0.0005, norm: str = "instance"):
        super(Ck, self).__init__(name=scope)
        self.norm = norm
        self.conv1 = conv(numout=k, kernel_size=3, strides=stride, kernel_regularizer=reg, padding='same',
                          name='conv')
        if norm == 'instance':
            self.scale = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='scale')
            self.offset = tf.Variable(initial_value=tf.random_normal_initializer(stddev=0.1)(shape=[k]), name='offset')
...
class last_conv(keras.Model):
    def __init__(self, scope: str = "last_conv", reg: float = 0.0005):
        ...
        self.conv1 = conv(numout=1, kernel_size=3, kernel_regularizer=reg, padding='same', name='conv')
