...
flags.DEFINE_integer('epoch', 150,
                     'weight for backward cycle loss (Y->X->Y), default: 10')
...
flags.DEFINE_integer('shape', 256,
                     'shape of img, default: 256')
flags.DEFINE_integer('channel', 1,
                     'channel of img, default: 1')
...
@tf.function
def train_step(G, F, D_Y, D_X, source, target, generator_g_optimizer, generator_f_optimizer,
               discriminator_x_optimizer, discriminator_y_optimizer, train_loss, lambda1, lambda2):
...
def lr_sch(epoch):
    if epoch + 1 <= 75:
        ...
        lr = tf.constant(2e-4 - 2e-8) * (1 - (epoch + 1 - 75) / 75) + 2e-8
    return lr
...
def main(_argv):
    ...
    G = generator.Generator('G', ngf)
    F = generator.Generator('F', ngf)
    D_Y = discriminator.Discriminator('D_Y')
    D_X = discriminator.Discriminator('D_X')
    forbuild = np.random.rand(1, shape, shape, channel).astype(np.float32)
    # d1 = tf.convert_to_tensor(np.random.rand(1, 128, 128, 32), dtype=tf.float32)
    # d2 = tf.convert_to_tensor(np.random.rand(1, 64, 64, 64), dtype=tf.float32)
    # d3 = tf.convert_to_tensor(np.random.rand(1, 32, 32, 128), dtype=tf.float32)
    built = G(forbuild)
    built = F(forbuild)
    built = D_Y(forbuild)
    built = D_X(forbuild)

    trainA_set = datarecoder.load_dataset(["trainA.tfrecords"])
    trainB_set = datarecoder.load_dataset(["trainB.tfrecords"])
    ...
    for ep in range(start, epoch, 1):
        ...
        for steps in range(270):
            tf.compat.v1.global_variables_initializer()
            source = iteratorA.get_next()
            target = iteratorB.get_next()
            fake_y, cycled_x, fake_x, cycled_y, same_x, same_y, total_gen_g_loss, total_gen_f_loss, disc_x_loss, disc_y_loss, steps = train_step(
                G, F, D_Y, D_X, source, target, generator_g_optimizer, generator_f_optimizer,
                discriminator_x_optimizer, discriminator_y_optimizer, train_loss, lambda1, lambda2)
